...
public class Consumer {
    ...
    public static void main(String[] args) throws IOException, TimeoutException {
        ...
        /**
         *  声明接收消息，这里模拟消费端收到消息，但是消费过程中发生异常；但是因为采取手动应答，此时消息又从MQ删除了，消息队列中的消息也就不会被删除了
         *  验证方式1：通过MQ管理界面，找到指定队列，可以看到main方法执行后，队列消息还存在
         *  验证方式2：启动另一个消费者从队列消费数据，能够正常渠道数据
         */
        DeliverCallback deliverCallback = (consumerTag, message) -> {
            System.out.println(new String(message.getBody()));
            System.out.println(10/0);
            //手动确认，消费端收到消息，还没处理完就发生异常，也就是没有消费成功，因为采取手动应答，这时候消费者不会向MQ应答，消息队列中的消息也就不会被删除了，
            //这样消息就能被消费者c2消费
