package com.example.mq.deadqueue.queue_length;
 ...
 *
 * 现象：生产者发布10条消息后，通过RabbitMQ web界面观察normal_queue只有6条数据，dead_queue，dead_queue中会存在4条数据，也就是超过正常队列长度的消息都转移到了死信队列
 * 注意创建同名队列，如果队里参数改变了，所以需要把原先队列删除
 */
public class ProducerMaxQueueLength {
    //普通交换机的名称
