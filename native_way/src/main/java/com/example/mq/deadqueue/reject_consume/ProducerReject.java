 ...
 *
 * 现象：生产者发布10条消息后，通过RabbitMQ web界面观察normal_queue，会发现10条数据，
 * 启动消费者，但是消费者只消费其中的9条消息，拒绝消费其中的一天消息，所以其中的一条被拒绝消费的数据转移到了dead_queue，
 * dead_queue中会存在1条数据，normal_queue变为空
